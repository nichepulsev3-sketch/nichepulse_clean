...
function Signal({ s, onClick }: { s: TrendSignal; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} title={`Buscar: ${s.keyword.replace('#','')}`}
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = cfg.color; (e.currentTarget as HTMLElement).style.background = 'var(--c4)' }}
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(255,255,255,0.07)'; (e.currentTarget as HTMLElement).style.background = 'var(--c3)' }}
    >
  ...
  return (
    <div style={{ marginBottom:'1rem' }}>
      <button onClick={() => setOpen(v=>!v)} style={{ width:'100%', background:'none', border:'none', cursor:'pointer', padding:'0 0 .5rem', display:'flex', alignItems:'center', justifyContent:'space-between', fontFamily:'var(--font-body)' }}>
        ...
        <span style={{ fontSize:12, color:'var(--t3)', transition:'transform .2s', display:'inline-block', transform: open ? 'rotate(0deg)' : 'rotate(-90deg)' }}>▾</span>
      </button>
      {open && (
...
export default function TrendsPanel({ geo='US', onKeywordClick }: { geo?: string; onKeywordClick?: (kw:string)=>void }) {
  ...
  const load = useCallback(async (force=false) => {
    try {
      ...
      const res = await fetch(`/api/trends?geo=${geo}${force?'&refresh=true':''}`)
      if (res.ok) setTrends(await res.json())
    } finally { setLoading(false); setRefreshing(false) }
  }, [geo])

  useEffect(() => { load() }, [load])
  useEffect(() => { const id = setInterval(() => load(true), 30*60*1000); return () => clearInterval(id) }, [load])
    ...
    <div style={{ background:'var(--c2)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:16, overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'12px 14px', borderBottom:'0.5px solid rgba(255,255,255,0.07)', background:'linear-gradient(135deg,rgba(124,111,255,0.08),rgba(255,107,157,0.05))', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:7, height:7, borderRadius:'50%', background:'#00e5c3', boxShadow:'0 0 6px #00e5c3', animation: loading?'none':'pulse 2s ease-in-out infinite' }} />
        ...
        <button onClick={() => load(true)} disabled={refreshing} title="Actualizar" style={{ background:'none', border:'none', cursor: refreshing?'wait':'pointer', color:'var(--t3)', fontSize:15, opacity: refreshing?.5:1, animation: refreshing?'spin .8s linear infinite':'none' }}>⟳</button>
      </div>
      ...
      <div style={{ padding:'12px 14px', flex:1, overflowY:'auto', maxHeight:500 }}>
        {loading ? (
          <div style={{ textAlign:'center', padding:'2rem' }}>
            ...
            <div style={{ fontSize:12, color:'var(--t3)' }}>Cargando señales...</div>
          </div>
        ) : trends ? (
          <>
          ...
          <div style={{ textAlign:'center', color:'var(--t3)', fontSize:13, padding:'2rem' }}>No se pudieron cargar las señales</div>
        )}
      </div>
